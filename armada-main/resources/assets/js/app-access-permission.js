...
$(function () {
  ...
  if (dataTableRoles.length) {
    dt_roles = dataTableRoles.DataTable({
      ajax: '/api/roles', // Asegúrate de que esta ruta devuelva los roles en formato JSON
      columns: [
        { data: 'id' },
        { data: 'name' },
        { data: 'created_at' },
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            return (
              '<button class="btn btn-icon btn-text-secondary delete-role" data-id="' + full['id'] + '"><i class="ti ti-trash ti-md"></i></button>'
            );
          }
        }
      ],
  ...
  // Delete Role
  $('.datatables-roles tbody').on('click', '.delete-role', function () {
