...
document.addEventListener('DOMContentLoaded', function(e) {
    if (typeof $ !== 'undefined') {
        ...
        (function() {
            ...
            // Función para obtener el TRM del día
            function fetchTrm() {
                fetch('https://api.exchangerate-api.com/v4/latest/USD')
                    .then(response => response.json())
                    .then(data => {
                        if (data.rates && data.rates.COP) {
                            document.getElementById('trm').value = data.rates.COP;
                        } else {
                            console.error('Error al obtener el TRM:', data.error);
                        }
                    })
                    .catch(error => console.error('Error en la solicitud:', error));
            }

            // Llama a la función para obtener el TRM al cargar la página
            fetchTrm();
        })();
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    const acquisitionForm = document.querySelector('.acquisition-form-validation');

    if (acquisitionForm) {
        const fv = FormValidation.formValidation(acquisitionForm, {
                fields: {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            Swal.fire({
                                title: '¡Éxito!',
                                text: 'Plan de adquisición creado exitosamente',
                                icon: 'success',
